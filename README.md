...
<head>
    ...
    <style>
        ...
        button { background-color: #4caf50; color: white; border: none; padding: 15px 30px; font-size: 1em; border-radius: 50px; cursor: pointer; transition: 0.3s; }
...
<div class="container">
    ...
    <button id="speak-btn" onclick="startListening()">🎤 Tap to Speak</button>

    <div id="result"></div>
    <div id="transcript" class="transcript"></div>
    
    <div style="margin-top:20px;">
        <button onclick="nextLevel()" style="background-color:#2196f3; font-size:0.9em; padding:10px 20px;">Next Sentence ➡️</button>
    </div>
</div>
...
<script>
    ...
    // Helper to remove punctuation for easier matching
    ...
    function updateUI() {
        if (currentIndex >= levels.length) {
             document.querySelector('.container').innerHTML = `
                ...
                <button onclick="location.reload()" style="background-color:#2196f3; color:white; padding:15px 30px; border:none; border-radius:50px; cursor:pointer;">Play Again 🔄</button>
             `;
             return;
        }
    ...
    recognition.onresult = (event) => {
        ...
        // Clean up both the speech and the targets
        ...
        if (isHardMode) {
            // HARD MODE: Must match the sentence exactly
            if (cleanSpeech === cleanTargetSentence) {
                passed = true;
            }
        } else {
            // STANDARD MODE: Just needs to contain the word
            if (cleanSpeech.includes(cleanTargetWord)) {
                passed = true;
            }
        }
        ...
        if (passed) {
            ...
            if (isHardMode) {
                document.getElementById('result').innerHTML = "<span class='fail'>❌ Try Again. (Must match sentence exactly!)</span>";
            } else {
                document.getElementById('result').innerHTML = "<span class='fail'>❌ Try Again. Keep it clear!</span>";
            }
        }
